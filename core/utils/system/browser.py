...
def browser_cookie_sources() -> tuple[str, ...]:
    """Return browser cookie stores suitable for the current platform."""
...
def isolated_browser_fallback_supported() -> bool:
    """The isolated browser fallback is currently Windows-only."""
    return platform.system() == "Windows"


def _headless_browser_candidates() -> list[Path]:
    if isolated_browser_fallback_supported():
        return [
...
def find_headless_browser() -> Path | None:
    if not isolated_browser_fallback_supported():
        return None
...
def dump_webpage_with_browser(url: str, *, timeout: int = 45) -> str:
    ...
    if not isolated_browser_fallback_supported():
        raise RuntimeError("当前系统不启用隔离浏览器回退")
    ...
    with tempfile.TemporaryDirectory(prefix="biliup-browser-", ignore_cleanup_errors=True) as profile:
        try:
            ...
            raise RuntimeError("隔离浏览器访问页面超时") from exc
