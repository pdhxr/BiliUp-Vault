...
当前 MVP：首次选择视频知识库目录后，通过 OpenCLI 搜索并登记 UP 主；“UP 主管理”页签支持刷新视频信息列表，第二个页签支持选择视频并下载到知识库，也支持按起始日期批量追踪并下载多个 UP 的未下载视频。下载成功后会尝试获取字幕脚本，并在视频列表显示是否已有字幕脚本。输入昵称不会自动搜索，只有点击“搜索”按钮才会调用 OpenCLI；搜索期间按钮置灰并显示“搜索中…”。搜索结果区默认显示约 3–5 项，其余结果可在列表内滚动查看；“确认写入”按钮位于结果列表上方。
...
- 视频旁的 `<视频文件名>__transcript.md` 是字幕脚本 sidecar，由 OpenCLI `bilibili subtitle` 生成；存在且非空时两套索引的 `transcript` 为 `true`。
- 刷新视频列表时，系统按网站元数据扫描已有月度视频，补建缺失的本地 `videos.jsonl` 条目，重新检查字幕脚本 sidecar，并用本地索引校准 `UpList/<UP名称>.jsonl` 的下载状态和 `transcript`。下载成功时会尝试获取字幕，字幕失败不影响视频下载。
...
页签从当前配置的 `<知识库目录>/UpList/followings.json` 读取登记数据，显示：选择框、序号、UP 名称、UP 简介、自动追踪下载、最后同步时间、视频总数、已下载/已同步和操作列。视频下载列表另外显示“有字幕脚本”列。当前登记数据没有视频统计时，统计列显示 `0 / 0`；未提供同步时间时显示 `-`。
