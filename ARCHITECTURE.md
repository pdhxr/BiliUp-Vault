...
当前 MVP 已实现首次知识库目录设置、OpenCLI 搜索、UP 单选登记、UP 视频列表刷新、选中视频下载和可选字幕 sidecar 接口；范围见 [PRD.md](PRD.md)。
...
- `core/subtitle_download.py`：调用 OpenCLI 获取字幕、写入 `__transcript.md` sidecar，并同步远端追踪清单和本地视频索引；接口失败不阻断视频下载。
