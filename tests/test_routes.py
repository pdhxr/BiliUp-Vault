...
class RouteTests(unittest.TestCase):
    ...
    @patch("app.main.open_browser_after_start")
    @patch("app.main.prepare_biliup_port", return_value=8765)
    @patch("app.main.stop_existing_biliup_services")
    def test_main_uses_fixed_biliup_port(self, stop_old, prepare_port, browser, run) -> None:
        main()
    ...
    @patch("app.routes.setup.open_directory")
    @patch("app.routes.setup.knowledge_base_root", return_value=Path("tmp/test-library"))
    def test_open_library_folder_route(self, _root, _open) -> None:
        response = self.client.post("/api/library/open-folder")
        self.assertEqual(response.status_code, 200)
    ...
    @patch("app.routes.setup.open_directory")
    @patch("app.routes.setup.knowledge_base_root", return_value=Path("tmp/test-library"))
    def test_open_single_video_folder_route(self, _root, _open) -> None:
        response = self.client.post("/api/single-video/open-folder")
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.json()["path"].endswith("OtherVideos"))
        _open.assert_called_once()
    ...
    def test_dashboard_contains_all_static_tabs(self) -> None:
        ...
        self.assertIn('class="status-checkbox"', video_script.text)
        self.assertIn('disabled ${video.transcript', video_script.text)
        self.assertIn('disabled ${video.cover', video_script.text)
        ...
        other_script = self.client.get("/static/js/other_features.js")
    ...
    @patch("app.routes.videos.start_batch_sync", return_value={"status": "started", "total": 1})
    def test_video_refresh_route(self, _start) -> None:
        response = self.client.post("/api/up/1/videos/refresh", json={"page": 1, "limit": 50})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["status"], "started")
        _start.assert_called_once_with(
            ["1"], max_pages=1, max_new_videos=30, limit=30, continue_history=True,
        )
    ...
    @patch("app.routes.videos.queue_single_video_download", return_value={"bvid": "BV1", "status": "queued"})
    def test_single_video_download_route(self, _queue) -> None:
        response = self.client.post("/api/single-video/download", json={"url": "BV1"})
        self.assertEqual(response.status_code, 200)
