...
- Windows 抖音单视频下载增加 Chrome、Edge、Firefox Cookie 与隔离浏览器回退，并保留 macOS 既有下载路径。
- B 站单视频下载支持从完整分享文本中提取短链接。
- 停止批量追踪下载时立即标记尚未执行的排队任务，界面同步刷新停止状态。
- 发布测试中的路径分隔符和模拟平台断言兼容 Windows 与 macOS。

### 验证状态

- Windows 11 的 140 项 Python 自动测试和 3 项 MSVC Rust 测试全部通过。
- Windows x64 的 NSIS、MSI 安装包已完成原生 MSVC 构建、安装和抖音单视频下载验证。
- Windows 安装包尚未进行代码签名，首次运行可能显示 SmartScreen 提示。
